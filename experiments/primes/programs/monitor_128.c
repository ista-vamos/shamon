...
stream_args_P_1 = malloc(sizeof(STREAM_Primes_ARGS));
	...
	BUFFER_P0 = shm_arbiter_buffer_create(EV_SOURCE_P_0,  sizeof(STREAM_Primes_out), 128);

	// connect to event source P_1
	EV_SOURCE_P_1 = shm_stream_create_from_argv("P_1", argc, argv);
	BUFFER_P1 = shm_arbiter_buffer_create(EV_SOURCE_P_1,  sizeof(STREAM_Primes_out), 128);
