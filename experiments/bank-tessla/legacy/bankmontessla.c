...
struct _STREAM_BankOutputEvent_in {
    shm_event head;
    union {
        EVENT_depositTo depositTo;EVENT_withdraw withdraw;EVENT_withdrawFail withdrawFail;EVENT_logout logout;EVENT_transferSuccess transferSuccess;EVENT_selectedAccount selectedAccount;EVENT_numOut numOut;EVENT_depositSuccess depositSuccess;EVENT_balance balance;EVENT_depositFail depositFail;EVENT_withdrawSuccess withdrawSuccess;EVENT_transfer transfer;
    }cases;
};
...
struct _STREAM_BankEvent_in {
    shm_event head;
    union {
        EVENT_monbalance monbalance;EVENT_mondeposit mondeposit;EVENT_monwithdraw monwithdraw;EVENT_monreset monreset;EVENT_montransfer montransfer;
    }cases;
};
...
extern void monwithdraw(void* mon, int acc, int amnt, int success,long timestamp);
extern void montransfer(void* mon, int src, int tgt, int amnt, int success, long timestamp);
...
if (count_Out >= 1) {	ok = 0;
	...
	print_buffer_prefix(BUFFER_Out, i1_Out + i2_Out, count_Out, e1_Out, i1_Out, e2_Out, i2_Out);
...
if (count_Out >= 1) {if (count_In >= 1) {	ok = 0;
	...
	print_buffer_prefix(BUFFER_In, i1_In + i2_In, count_In, e1_In, i1_In, e2_In, i2_In);
fprintf(stderr, "No rule matched even though there was enough events, NO PROGRESS!\n");assert(0);}
...
int amnt = event_for_amnt->cases.numIn.num;
    ...
    arbiter_outevent->head.id = (*arbiter_counter)++;
    ((STREAM_BankEvent_out *) arbiter_outevent)->cases.mondeposit.account = acc;
((STREAM_BankEvent_out *) arbiter_outevent)->cases.mondeposit.amount = amnt;
...
int main(int argc, char **argv) {
 ...
 init_intmap(&lower_bounds);
	...
	BUFFER_In = shm_arbiter_buffer_create(EV_SOURCE_In,  sizeof(STREAM_BankInputEvent_out), 128);
	...
	BUFFER_Out = shm_arbiter_buffer_create(EV_SOURCE_Out,  sizeof(STREAM_BankOutputEvent_out), 128);
	...
	shm_arbiter_buffer_set_active(BUFFER_Out, true);
 	...
 	monitor_buffer = shm_monitor_buffer_create(sizeof(STREAM_BankEvent_out), 128);
