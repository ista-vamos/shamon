...
char *buf_get_line(msgbuf *buf)
{
	...
	do
	{
		char *nlpos = strstr(current->textbuf, "\n");
		if (nlpos != NULL)
		{
			intptr_t len = (((intptr_t)nlpos) - ((intptr_t)current->textbuf)) + 1;
			...
			{
				char *newbuf = (char *)malloc(curlen - len + 1 + sizeof(size_t) + sizeof(int64_t));
				newbuf+=sizeof(size_t)+sizeof(int64_t);
				memcpy(newbuf, current->textbuf + len, curlen - len);
				newbuf[curlen - len] = 0;
				free(current->textbuf - sizeof(size_t) - sizeof(int64_t));
				current->textbuf = newbuf;
				if (current == buf)
				{
					if (current->next != current)
					{
