...
void checkBalance(char *line, int parts)
{
	...
	if(processLogin(&line,&user,&account))
	{
		printf("Balance on account %lu: %lu\n",account->number,account->ops->checkBalance(account));
	}
}
void transfer(char *line, int parts)
{
	if(parts!=6)
	{
		printf("Usage: transfer [user] [pwd] [account] [receiver-account] [amount]\n");
		return;
	}
	User* user = NULL;
	Account* account = NULL;
	if(processLogin(&line,&user,&account))
	{
		...
		if(receiver!=NULL)
		{
			...
			if(amount>0)
			{
				if(account->number,account->ops->transfer(account, receiver, amount))
				{
	...
	while(!stop)
	{
		...
		if(line!=NULL)
		{
			...
			else if(strcmp(line, "help")==0)
			{
				...
				printf("transfer [user] [pwd] [account] [receiver-account] [amount]\n");
				printf("help\n");
				printf("quit\n\n");
			}
