...
char *buf_get_upto(msgbuf *buf, char *delim) {
    ...
    do {
        ...
        if (nlpos != NULL) {
            intptr_t len = (((intptr_t)nlpos) - ((intptr_t)current->textbuf)) +
                           strlen(delim);
        ...
        if (nextline != NULL) {
            ...
            if (strncmp(prompt + 27, "10000", 5) == 0) {
                ...
                if (acctidx >= 0 && acctidx < 4) {
                    if (acct_balances[acctidx] >= 0 &&
                        acct_balances[acctidx] != amount) {
                        printf("WARNING: found mismatched account balance for "
                               "account 10000%i. Expected %li, but found %li\n",
                               acctidx, acct_balances[acctidx], amount);
                    } else if (acct_balances[acctidx] >= 0) {
                        if (do_print) {
                            printf("Checked account balance for account "
                                   "10000%i; result %li matches expectation",
                                   acctidx, amount);
                        }
                    } else {
                        if (do_print) {
            ...
            while (line != NULL) {
                ...
                if (strlen(line) <= 1 || strncmp(line, "Welcome, ", 9) == 0) {
                    ...
                    if (acct_balances[pending_withdrawal_source] >= 0) {
                        if (acct_balances[pending_withdrawal_source] >=
                            pending_withdrawal) {
                            acct_balances[pending_withdrawal_source] -=
                                pending_withdrawal;
                        } else {
                            printf("WARNING: Withdrawal of %li from account "
                                   "10000%i allowed despite expected balance "
                                   "of %li!\n",
                                   pending_withdrawal,
                                   pending_withdrawal_source,
                                   acct_balances[pending_withdrawal_source]);
                            acct_balances[pending_withdrawal_source] = -1;
                        }
                    } else if (acct_limits[pending_withdrawal_source] >= 0) {
                        if (acct_limits[pending_withdrawal_source] >=
                            pending_withdrawal) {
                            acct_limits[pending_withdrawal_source] -=
                                pending_withdrawal;
                        } else {
                            printf("WARNING: Withdrawal of %li from account "
                                   "10000%i allowed despite previously "
                                   "determined upper balance limit of %li!\n",
                                   pending_withdrawal,
                                   pending_withdrawal_source,
                                   acct_limits[pending_withdrawal_source]);
                    ...
                    if (acct_changes[pending_withdrawal_source] >
                        pending_withdrawal) {
                        acct_changes[pending_withdrawal_source] -=
                            pending_withdrawal;
                    } else {
                    ...
                    if (acct_balances[pending_withdrawal_source] >= 0) {
                        if (acct_balances[pending_withdrawal_source] >=
                            pending_withdrawal) {
                            printf("WARNING: Withdrawal of %li from account "
                                   "10000%i failed despite expected balance of "
                                   "%li!\n",
                                   pending_withdrawal,
                                   pending_withdrawal_source,
                                   acct_balances[pending_withdrawal_source]);
                        ...
                        if (acct_limits[pending_withdrawal_source] >=
                                pending_withdrawal ||
                            acct_limits[pending_withdrawal_source] < 0) {
                            acct_limits[pending_withdrawal_source] =
                                pending_withdrawal - 1;
                        }
                        if (acct_changes[pending_withdrawal_source] >=
                            pending_withdrawal) {
                            printf(
                                "WARNING: Withdrawal of %li from account "
                                "10000%i failed despite previously determined "
                                "lower balance bound of of %li!\n",
                                pending_withdrawal, pending_withdrawal_source,
                                acct_changes[pending_withdrawal_source]);
                        }
                    }
                    pending_withdrawal        = -1;
                ...
                if (strlen(line) <= 1 || strncmp(line, "Welcome, ", 9) == 0) {
                    ...
                    if (pending_transfer_source >= 0) {
                        if (acct_balances[pending_transfer_source] >= 0) {
                            if (acct_balances[pending_transfer_source] >=
                                pending_transfer) {
                                acct_balances[pending_transfer_source] -=
                                    pending_transfer;
                            } else {
                                printf(
                                    "WARNING: Transfer of %li from account "
                                    "1000%i to account 1000%i allowed despite "
                                    "insufficient expected balance %li!\n",
                                    pending_transfer, pending_transfer_source,
                            ...
                            acct_changes[pending_transfer_source] +=
                                pending_transfer;
                            if (acct_limits[pending_transfer_source] >= 0) {
                                if (acct_limits[pending_transfer_source] >=
                                    pending_transfer) {
                                    acct_limits[pending_transfer_source] -=
                                        pending_transfer;
                                } else {
                                    printf(
                                        "WARNING: Transfer of %li from account "
                                        "1000%i to account 1000%i allowed "
                                        "despite previously determined upper "
                                        "balance limit %li!\n",
                                        pending_transfer,
                            ...
                            if (acct_changes[pending_transfer_source] >=
                                pending_transfer) {
                                acct_changes[pending_transfer_source] -=
                                    pending_transfer;
                            } else {
                    ...
                    if (pending_transfer_target >= 0) {
                        if (acct_balances[pending_transfer_target] >= 0) {
                            acct_balances[pending_transfer_target] +=
                                pending_transfer;
                        } else if (acct_limits[pending_transfer_target] >= 0) {
                            acct_limits[pending_transfer_target] +=
                                pending_transfer;
                        } else {
                            acct_changes[pending_transfer_target] +=
                                pending_transfer;
                        }
                    }
                    pending_transfer        = -1;
                    ...
                    if (pending_transfer_source >= 0) {
                        if (acct_balances[pending_transfer_source] >=
                            pending_transfer) {
                            printf("WARNING: Transfer of %li from account "
                                   "1000%i to account 1000%i failed despite "
                                   "previously determined balance %li!\n",
                                   pending_transfer, pending_transfer_source,
                                   pending_transfer_target,
                                   acct_balances[pending_transfer_source]);
                            acct_balances[pending_transfer_source] = -1;
                        ...
                        if (acct_changes[pending_transfer_source] >=
                            pending_transfer) {
                            printf(
                                "WARNING: Transfer of %li from account 1000%i "
                                "to account 1000%i failed despite previously "
                                "determined lower balance bound %li!\n",
                                pending_transfer, pending_transfer_source,
                                pending_transfer_target,
                                acct_changes[pending_transfer_source]);
                            acct_changes[pending_transfer_source] = 0;
                        ...
                        if (acct_limits[pending_transfer_source] >=
                                pending_transfer ||
                            acct_limits[pending_transfer_source] < 0) {
                            acct_limits[pending_transfer_target] =
                                pending_transfer - 1;
                        }
                    }
