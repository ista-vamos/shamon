...
void printevent(FILE *file, event *ev) {
    ...
    case EV_CHECKBALANCE:
        fprintf(file, "CHECKBALANCE(%lu, %lu)\n", ev->data.checkbalance.account,
                ev->data.checkbalance.amount);
        break;
    case EV_DEPOSIT:
        fprintf(file, "DEPOSIT(%lu, %lu)\n", ev->data.deposit.account,
                ev->data.deposit.amount);
        break;
    case EV_WITHDRAW:
        fprintf(file, "WITHDRAW(%lu, %lu)\n", ev->data.withdraw.account,
                ev->data.withdraw.amount);
        break;
    case EV_WITHDRAW_FAIL:
        ...
        break;
    case EV_TRANSFER:
        fprintf(file, "TRANSFER(%lu, %lu, %lu)\n", ev->data.transfer.account,
                ev->data.transfer.target, ev->data.transfer.amount);
        break;
    case EV_TRANSFER_FAIL:
        fprintf(file, "TRANSFERFAIL(%lu, %lu, %lu)\n",
                ev->data.transfer.account, ev->data.transfer.target,
                ev->data.transfer.amount);
        break;
    }
}
...
event *get_next_event_place() {
    if (evwritepos % EVBUFSIZE == 0) {
        if (localevcount >= EVBUFSIZE * 2) {
            ...
            if (localevcount >= EVBUFSIZE * 2) {
                localevcount = atomic_fetch_sub_explicit(&evcount, EVBUFSIZE,
                                                         memory_order_acq_rel) -
                               EVBUFSIZE;
                if (localevcount < EVBUFSIZE) {
                    localevcount =
                        atomic_fetch_add_explicit(&evcount, EVBUFSIZE,
                                                  memory_order_acq_rel) +
                        EVBUFSIZE;
...
void commit_event() {
    ...
    localevcount =
        atomic_fetch_add_explicit(&evcount, countadd, memory_order_acq_rel) +
        countadd;
...
void record_failed_transfer_event(uint64_t account, uint64_t target,
                                  uint64_t amount) {
    if (do_print) {
...
int process_events_thread(void *arg) {
    ...
    Z3_func_decl accidfunc =
        Z3_mk_func_decl(ctx, accidname, 1, accid_domain, intsort);
    Z3_func_decl balancefunc =
        Z3_mk_func_decl(ctx, balancename, 2, balance_domain, intsort);
    Z3_func_decl worldidfunc =
        Z3_mk_func_decl(ctx, worldidname, 1, &worldsort, intsort);
    ...
    Z3_ast accids_eq_acc_eq_forall =
        Z3_mk_forall(ctx, 0, /*1, &accid0_1_pattern,*/ 0, 0, 2, accounts2,
                     accountsyms2, accids_eq_acc_eq);
    ...
    Z3_ast worldids_eq_acc_eq_forall =
        Z3_mk_forall(ctx, 0, /*1, &worldid0_1_pattern,*/ 0, 0, 2, worlds2,
                     worldsyms2, worldids_eq_acc_eq);
    ...
    Z3_ast accbalance_ge_0 =
        Z3_mk_ge(ctx, accbalance0_1, Z3_mk_int(ctx, 0, intsort));
    ...
    Z3_ast     balance_gt_0_forall =
        Z3_mk_forall(ctx, 0, /*1, &accbalance_pattern,*/ 0, 0, 2,
                     account_world_sorts, accworldsym2, accbalance_ge_0);
    ...
    Z3_ast    initworldid_eq_0 =
        Z3_mk_eq(ctx, initworldid, Z3_mk_int(ctx, 0, intsort));
    ...
    while (monitoring_active) {
        if (remaining <= 0) {
            ...
            while (remaining <= 0 && monitoring_active) {
                ...
                remaining =
                    atomic_load_explicit(&evcount, memory_order_acquire);
            }
            mtx_unlock(&evwait_mutex);
        }
        ...
        while (remaining > 0) {
            ...
            case EV_CHECKBALANCE: {
                ...
                Z3_ast currentaccountid =
                    Z3_mk_int(ctx, current.data.checkbalance.account, intsort);
                Z3_ast worldid_past =
                    Z3_mk_eq(ctx, worldid0, Z3_mk_int(ctx, worlds, intsort));
                worlds++;
                Z3_ast worldid_new =
                    Z3_mk_eq(ctx, worldid1, Z3_mk_int(ctx, worlds, intsort));

                Z3_ast accbalance_old_args[2] = {accountbound0, worldbound1};
                Z3_ast accbalance_new_args[2] = {accountbound0, worldbound2};
                Z3_ast accbalance_old =
                    Z3_mk_app(ctx, balancefunc, 2, accbalance_old_args);
                Z3_ast accbalance_new =
                    Z3_mk_app(ctx, balancefunc, 2, accbalance_new_args);

                Z3_ast balances_eq =
                    Z3_mk_eq(ctx, accbalance_old, accbalance_new);
                Z3_ast balance_check = Z3_mk_implies(
                    ctx, Z3_mk_eq(ctx, accid0, currentaccountid),
                    Z3_mk_eq(ctx, accbalance_new,
                             Z3_mk_int(ctx, current.data.checkbalance.amount,
                                       intsort)));
                Z3_ast forall_ands[2]  = {balances_eq, balance_check};
                Z3_ast forall_body     = Z3_mk_and(ctx, 2, forall_ands);
                Z3_ast balances_forall = Z3_mk_forall(
                    ctx, 0, 0, NULL, 1, &accountsort, &accsym, forall_body);

                Z3_ast ex_ands[3]   = {worldid_past, worldid_new,
                                       balances_forall};
                Z3_ast ex_body      = Z3_mk_and(ctx, 3, ex_ands);
                Z3_ast exist_worlds = Z3_mk_exists(ctx, 0, 0, NULL, 2, worlds2,
                                                   worldsyms2, ex_body);
                Z3_solver_assert(ctx, s, exist_worlds);

                Z3_ast exists_account = Z3_mk_exists(
                    ctx, 0, 0, 0, 1, &accountsort, &accsym,
                    Z3_mk_eq(ctx, Z3_mk_app(ctx, accidfunc, 1, &accountbound0),
                             currentaccountid));
                ...
                if (Z3_solver_check(ctx, s) == Z3_L_FALSE) {
                    ...
                    printf("Mismatch found at checkbalance event: %lu in %lu\n",
                           current.data.checkbalance.amount,
                           current.data.checkbalance.account);
                    worlds = 0;
            ...
            case EV_DEPOSIT: {
                ...
                Z3_ast currentaccountid =
                    Z3_mk_int(ctx, current.data.deposit.account, intsort);
                Z3_ast worldid_past =
                    Z3_mk_eq(ctx, worldid0, Z3_mk_int(ctx, worlds, intsort));
                worlds++;
                Z3_ast worldid_new =
                ...
                Z3_ast add_args[2] = {
                    accbalance_old,
                    Z3_mk_int(ctx, current.data.deposit.amount, intsort)};
                Z3_ast and_args[2] = {
                    Z3_mk_eq(ctx, accid0, currentaccountid),
                    Z3_mk_eq(ctx, accbalance_new, Z3_mk_add(ctx, 2, add_args))};
            ...
            case EV_WITHDRAW: {
                ...
                Z3_ast currentaccountid =
                    Z3_mk_int(ctx, current.data.withdraw.account, intsort);
                Z3_ast worldid_past =
                    Z3_mk_eq(ctx, worldid0, Z3_mk_int(ctx, worlds, intsort));
                worlds++;
                Z3_ast worldid_new =
                ...
                Z3_ast sub_args[2] = {
                    accbalance_old,
                    Z3_mk_int(ctx, current.data.withdraw.amount, intsort)};
                Z3_ast and_args[2] = {
                    Z3_mk_eq(ctx, accid0, currentaccountid),
                    Z3_mk_eq(ctx, accbalance_new, Z3_mk_sub(ctx, 2, sub_args))};
                Z3_ast balance_add     = Z3_mk_and(ctx, 2, and_args);
                Z3_ast forall_ors[2]   = {balances_eq, balance_add};
                Z3_ast forall_body     = Z3_mk_or(ctx, 2, forall_ors);
                Z3_ast balances_forall = Z3_mk_forall(
            ...
            case EV_WITHDRAW_FAIL: {
                ...
                if (Z3_solver_check(ctx, s) == Z3_L_FALSE) {
                    ...
                    Z3_solver_pop(ctx, s, 1);
                    Z3_solver_push(ctx, s);
                    printf("Mismatch found at failed withdraw event: %lu from "
                           "%lu\n",
                           current.data.checkbalance.amount,
                           current.data.checkbalance.account);
                    worlds = 0;
            ...
            case EV_TRANSFER: {
                ...
                Z3_ast currentaccountid =
                    Z3_mk_int(ctx, current.data.transfer.account, intsort);
                Z3_ast targetaccountid =
                    Z3_mk_int(ctx, current.data.transfer.account, intsort);
                Z3_ast worldid_past =
                    Z3_mk_eq(ctx, worldid0, Z3_mk_int(ctx, worlds, intsort));
                worlds++;
                ...
                Z3_ast sub_args[2] = {
                    accbalance_old,
                    Z3_mk_int(ctx, current.data.transfer.amount, intsort)};
                Z3_ast and_args[2] = {
                    Z3_mk_eq(ctx, accid0, currentaccountid),
                    Z3_mk_eq(ctx, accbalance_new, Z3_mk_sub(ctx, 2, sub_args))};
                Z3_ast add_args[2] = {
                    accbalance_old,
                    Z3_mk_int(ctx, current.data.transfer.amount, intsort)};
                Z3_ast and_args_target[2] = {
                    Z3_mk_eq(ctx, accid0, targetaccountid),
                    Z3_mk_eq(ctx, accbalance_new, Z3_mk_add(ctx, 2, add_args))};
                Z3_ast balance_add        = Z3_mk_and(ctx, 2, and_args);
                Z3_ast balance_add_target = Z3_mk_and(ctx, 2, and_args_target);

                Z3_ast forall_ors[3]   = {balances_eq, balance_add,
                                          balance_add_target};
                ...
                Z3_ast exists_target_account = Z3_mk_exists(
                    ctx, 0, 0, 0, 1, &accountsort, &accsym,
                    Z3_mk_eq(ctx, Z3_mk_app(ctx, accidfunc, 1, &accountbound0),
                             targetaccountid));
                ...
                if (Z3_solver_check(ctx, s) == Z3_L_FALSE) {
                    ...
                    printf("Mismatch found at transfer event: %lu from %lu to "
                           "%lu\n",
                           current.data.transfer.amount,
                           current.data.transfer.account,
                           current.data.transfer.target);
                    Z3_solver_pop(ctx, s, 1);
                    Z3_solver_push(ctx, s);
                    worlds = 0;
                } else if (do_print_smt) {
            ...
            case EV_TRANSFER_FAIL: {
                ...
                if (Z3_solver_check(ctx, s) == Z3_L_FALSE) {
                    ...
                    Z3_solver_pop(ctx, s, 1);
                    Z3_solver_push(ctx, s);
                    printf("Mismatch found at failed transfer event: %lu from "
                           "%lu\n",
                           current.data.checkbalance.amount,
                           current.data.checkbalance.account);
                    worlds = 0;
            ...
            processed++;
            if (atomic_compare_exchange_strong(&evpos, &current_evpos,
                                               newpos)) {
                ...
                if (evprocesslog != NULL) {
                    fprintf(evprocesslog,
                            "Skipping ahead in process log to %i\n",
                            current_evpos);
                }
                break;
            }
        }
        remaining = atomic_fetch_sub_explicit(&evcount, processed,
                                              memory_order_acq_rel) -
                    processed;
    }
}
    ...
    while (prefix_left > 0) {
        ...
        if (strncmp(current->textbuf, prefix, cmplen) == 0) {
            ...
            if (prefix_left > 0) {
                current = current->next;
                if (current == buf) {
                    return prefix_orig; // indicates that prefix matches so far,
                                        // but no more data available; try again
                                        // later
                }
            } else {
...
typedef enum _processing_state {
    PS_NOTHING,
    PS_NOTHING_RESPONSE,
    PS_DEPOSIT,
    PS_WITHDRAWAL,
    PS_WITHDRAWAL_RESULT,
    PS_WITHDRAWAL_RESULT_LINE2,
    PS_TRANSFER,
    PS_TRANSFER_TARGET,
    PS_TRANSFER_AMOUNT,
    PS_TRANSFER_RESULT,
    PS_TRANSFER_RESULT_LINE2,
    PS_CHECKBALANCE
} processing_state;
