...
int RULE_SET_working() {
    ...
    if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, count_Out,
        ...
        if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, count_In,
                               sizeof(STREAM_BankInputEvent_out), TEMPARR55,
                               2)) {
    ...
    int TEMPARR56[] = {BANKOUTPUTEVENT_WITHDRAW,
                       BANKOUTPUTEVENT_WITHDRAWSUCCESS};
    int TEMPARR57[] = {BANKINPUTEVENT_NUMIN, BANKINPUTEVENT_OTHERIN};

    if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, count_Out,
                           sizeof(STREAM_BankOutputEvent_out), TEMPARR56, 2)) {

        if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, count_In,
                               sizeof(STREAM_BankInputEvent_out), TEMPARR57,
                               2)) {
    ...
    if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, count_Out,
                           sizeof(STREAM_BankOutputEvent_out), TEMPARR58, 2)) {
                ...
                if (intmap_get(&balances, account, &old_value)) {
                    ...
                    if (intmap_get(&upper_bounds, account, &old_value)) {
                        if (old_value >= amount) {
                            ...
                            ++errors;
                            printf("Withdrawal of %i from account #%i should "
                                   "have failed "
                ...
                if (intmap_get(&balances, from_, &old_value)) {
                    ...
                    if (intmap_get(&upper_bounds, from_, &old_value)) {
                        if (old_value >= amount) {
                            ...
                            ++errors;
                            printf("Transfer of %i from account #%i should "
                                   "have failed "
                                   "based on upper balance bound %i\n",
                                   amount, from_, old_value);
