...
struct _STREAM_BankOutputEvent_in {
    shm_event head;
    union {
        EVENT_balance balance;EVENT_logout logout;EVENT_withdrawFail withdrawFail;EVENT_transferSuccess transferSuccess;EVENT_transfer transfer;EVENT_numOut numOut;EVENT_withdraw withdraw;EVENT_depositTo depositTo;EVENT_depositSuccess depositSuccess;EVENT_depositFail depositFail;EVENT_withdrawSuccess withdrawSuccess;EVENT_selectedAccount selectedAccount;
    }cases;
};
...
struct _EVENT_deposit {
	int account;
	int amount;

};
typedef struct _EVENT_deposit EVENT_deposit;
...
typedef struct _EVENT_reset EVENT_reset;

// input stream for stream type BankEvent
struct _STREAM_BankEvent_in {
    shm_event head;
    union {
        EVENT_monbalance monbalance;EVENT_deposit deposit;EVENT_monwithdraw monwithdraw;EVENT_reset reset;EVENT_montransfer montransfer;
    }cases;
};
...
size_t i1, i2;
	...
	count = shm_arbiter_buffer_peek(BUFFER_In, 10, (void**)&e1, &i1, (void**)&e2, &i2);
	print_buffer_prefix(BUFFER_In, 0, i1 + i2, count, e1, i1, e2, i2);
	fprintf(stderr, "Prefix of 'Out':\n");
	count = shm_arbiter_buffer_peek(BUFFER_Out, 10, (void**)&e1, &i1, (void**)&e2, &i2);
	print_buffer_prefix(BUFFER_Out, 1, i1 + i2, count, e1, i1, e2, i2);

}    


int RULE_SET_aligning() {
char* e1_Out; size_t i1_Out; char* e2_Out; size_t i2_Out;
int count_Out = shm_arbiter_buffer_peek(BUFFER_Out, 1, (void**)&e1_Out, &i1_Out, (void**)&e2_Out, &i2_Out);

            int TEMPARR0[] = {BANKOUTPUTEVENT_HOLE};

            
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR0, 1)) {
                    
            STREAM_BankOutputEvent_out * event_for_n = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 0);
int n = event_for_n->cases.hole.n;
        ...
        arbiter_outevent = (STREAM_BankEvent_out *)shm_monitor_buffer_write_ptr(monitor_buffer);
...
++out_holes;
            ...
            int TEMPARR1[] = {BANKOUTPUTEVENT_BALANCE};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR1, 1)) {
                    
            STREAM_BankOutputEvent_out * event_for_t = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 0);
uint64_t t = event_for_t->cases.balance.timestamp;

STREAM_BankOutputEvent_out * event_for_a = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 0);
int a = event_for_a->cases.balance.account;
            ...
            int TEMPARR2[] = {BANKOUTPUTEVENT_DEPOSITTO};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR2, 1)) {
...
int a = event_for_a->cases.depositTo.account;
            ...
            int TEMPARR3[] = {BANKOUTPUTEVENT_WITHDRAW};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR3, 1)) {
...
int a = event_for_a->cases.withdraw.account;
            ...
            int TEMPARR4[] = {BANKOUTPUTEVENT_TRANSFER};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR4, 1)) {
                    
            STREAM_BankOutputEvent_out * event_for_t = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 0);
uint64_t t = event_for_t->cases.transfer.timestamp;

STREAM_BankOutputEvent_out * event_for_s = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 0);
int s = event_for_s->cases.transfer.fromAccount;

STREAM_BankOutputEvent_out * event_for_r = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 0);
...
if (count_Out >= 1) {	ok = 0;
	...
	fprintf(stderr, "Prefix of 'Out':\n");
	count_Out = shm_arbiter_buffer_peek(BUFFER_Out, 5, &e1_Out, &i1_Out, &e2_Out, &i2_Out);
	print_buffer_prefix(BUFFER_Out, 1, i1_Out + i2_Out, count_Out, e1_Out, i1_Out, e2_Out, i2_Out);
fprintf(stderr, "No rule in rule set '{rule_set_name}' matched even though there was enough events, CYCLING WITH NO PROGRESS!\n");abort();}
if (++RULE_SET_aligning_nomatch_cnt > 8000000) {        	RULE_SET_aligning_nomatch_cnt = 0;	fprintf(stderr, "\033[31mRule set 'aligning' cycles long time without progress\033[0m\n");	fprintf(stderr, "Prefix of 'Out':\n");
	count_Out = shm_arbiter_buffer_peek(BUFFER_Out, 5, &e1_Out, &i1_Out, &e2_Out, &i2_Out);
	print_buffer_prefix(BUFFER_Out, 1, i1_Out + i2_Out, count_Out, e1_Out, i1_Out, e2_Out, i2_Out);
...
char* e1_Out; size_t i1_Out; char* e2_Out; size_t i2_Out;
int count_Out = shm_arbiter_buffer_peek(BUFFER_Out, 1, (void**)&e1_Out, &i1_Out, (void**)&e2_Out, &i2_Out);
char* e1_In; size_t i1_In; char* e2_In; size_t i2_In;
int count_In = shm_arbiter_buffer_peek(BUFFER_In, 1, (void**)&e1_In, &i1_In, (void**)&e2_In, &i2_In);
...
int TEMPARR6[] = {BANKINPUTEVENT_NUMIN};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR5, 1)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR6, 1)) {
                    
            STREAM_BankOutputEvent_out * event_for_tout = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 0);
uint64_t tout = event_for_tout->cases.balance.timestamp;

STREAM_BankOutputEvent_out * event_for_account = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 0);
int account = event_for_account->cases.balance.account;

STREAM_BankInputEvent_out * event_for_tin = (STREAM_BankInputEvent_out *) get_event_at_index(e1_In, i1_In, e2_In, i2_In, sizeof(STREAM_BankInputEvent_out), 0);
uint64_t tin = event_for_tin->cases.numIn.timestamp;

STREAM_BankInputEvent_out * event_for_action = (STREAM_BankInputEvent_out *) get_event_at_index(e1_In, i1_In, e2_In, i2_In, sizeof(STREAM_BankInputEvent_out), 0);
...
int TEMPARR8[] = {BANKINPUTEVENT_NUMIN};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR7, 1)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR8, 1)) {
...
int TEMPARR10[] = {BANKINPUTEVENT_NUMIN};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR9, 1)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR10, 1)) {
...
int TEMPARR12[] = {BANKINPUTEVENT_NUMIN};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR11, 1)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR12, 1)) {
                    
            STREAM_BankOutputEvent_out * event_for_tout = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 0);
uint64_t tout = event_for_tout->cases.transfer.timestamp;

STREAM_BankOutputEvent_out * event_for_from_ = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 0);
int from_ = event_for_from_->cases.transfer.fromAccount;

STREAM_BankOutputEvent_out * event_for_to_ = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 0);
...
current_rule_set = SWITCH_TO_RULE_SET_aligning;
            ...
            int TEMPARR13[] = {BANKOUTPUTEVENT_LOGOUT};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR13, 1)) {
...
uint64_t t = event_for_t->cases.logout.timestamp;
            ...
            int TEMPARR14[] = {BANKOUTPUTEVENT_NUMOUT};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR14, 1)) {
...
int n = event_for_n->cases.numOut.num;
            ...
            int TEMPARR15[] = {BANKOUTPUTEVENT_SELECTEDACCOUNT};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR15, 1)) {
...
int a = event_for_a->cases.selectedAccount.account;
            ...
            int TEMPARR16[] = {BANKOUTPUTEVENT_DEPOSITSUCCESS};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR16, 1)) {
...
uint64_t t = event_for_t->cases.depositSuccess.timestamp;
            ...
            int TEMPARR17[] = {BANKOUTPUTEVENT_DEPOSITFAIL};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR17, 1)) {
...
uint64_t t = event_for_t->cases.depositFail.timestamp;
            ...
            int TEMPARR18[] = {BANKOUTPUTEVENT_WITHDRAWSUCCESS};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR18, 1)) {
...
uint64_t t = event_for_t->cases.withdrawSuccess.timestamp;
            ...
            int TEMPARR19[] = {BANKOUTPUTEVENT_WITHDRAWFAIL};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR19, 1)) {
...
uint64_t t = event_for_t->cases.withdrawFail.timestamp;
            ...
            int TEMPARR20[] = {BANKOUTPUTEVENT_TRANSFERSUCCESS};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR20, 1)) {
...
uint64_t t = event_for_t->cases.transferSuccess.timestamp;
            ...
            if (check_at_least_n_events(count_Out, 1)) {
                ...
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR21, 1)) {
                    
            STREAM_BankInputEvent_out * event_for_n = (STREAM_BankInputEvent_out *) get_event_at_index(e1_In, i1_In, e2_In, i2_In, sizeof(STREAM_BankInputEvent_out), 0);
int n = event_for_n->cases.hole.n;
...
++in_holes;
 	    ++in_processed;
            ...
            if (check_at_least_n_events(count_Out, 1)) {
                ...
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR22, 1)) {
...
current_rule_set = SWITCH_TO_RULE_SET_aligning;
            ...
            _Bool ok = 1;
if (count_Out >= 1) {if (count_In >= 1) {	ok = 0;
}}if (ok == 0) {
	fprintf(stderr, "Prefix of 'Out':\n");
	count_Out = shm_arbiter_buffer_peek(BUFFER_Out, 5, &e1_Out, &i1_Out, &e2_Out, &i2_Out);
	print_buffer_prefix(BUFFER_Out, 1, i1_Out + i2_Out, count_Out, e1_Out, i1_Out, e2_Out, i2_Out);
	fprintf(stderr, "Prefix of 'In':\n");
	count_In = shm_arbiter_buffer_peek(BUFFER_In, 5, &e1_In, &i1_In, &e2_In, &i2_In);
	print_buffer_prefix(BUFFER_In, 0, i1_In + i2_In, count_In, e1_In, i1_In, e2_In, i2_In);
fprintf(stderr, "No rule in rule set '{rule_set_name}' matched even though there was enough events, CYCLING WITH NO PROGRESS!\n");abort();}
if (++RULE_SET_align_in_nomatch_cnt > 8000000) {        	RULE_SET_align_in_nomatch_cnt = 0;	fprintf(stderr, "\033[31mRule set 'align_in' cycles long time without progress\033[0m\n");	fprintf(stderr, "Prefix of 'Out':\n");
	count_Out = shm_arbiter_buffer_peek(BUFFER_Out, 5, &e1_Out, &i1_Out, &e2_Out, &i2_Out);
	print_buffer_prefix(BUFFER_Out, 1, i1_Out + i2_Out, count_Out, e1_Out, i1_Out, e2_Out, i2_Out);
	fprintf(stderr, "Prefix of 'In':\n");
	count_In = shm_arbiter_buffer_peek(BUFFER_In, 5, &e1_In, &i1_In, &e2_In, &i2_In);
...
char* e1_Out; size_t i1_Out; char* e2_Out; size_t i2_Out;
int count_Out = shm_arbiter_buffer_peek(BUFFER_Out, 2, (void**)&e1_Out, &i1_Out, (void**)&e2_Out, &i2_Out);
char* e1_In; size_t i1_In; char* e2_In; size_t i2_In;
int count_In = shm_arbiter_buffer_peek(BUFFER_In, 3, (void**)&e1_In, &i1_In, (void**)&e2_In, &i2_In);

            int TEMPARR23[] = {BANKOUTPUTEVENT_TRANSFER,BANKOUTPUTEVENT_TRANSFERSUCCESS};
int TEMPARR24[] = {BANKINPUTEVENT_NUMIN,BANKINPUTEVENT_NUMIN,BANKINPUTEVENT_NUMIN};

            
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR23, 2)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR24, 3)) {
                    
            STREAM_BankOutputEvent_out * event_for_t2 = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 0);
...
STREAM_BankOutputEvent_out * event_for_t4 = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 1);
uint64_t t4 = event_for_t4->cases.transferSuccess.timestamp;

STREAM_BankInputEvent_out * event_for_t0 = (STREAM_BankInputEvent_out *) get_event_at_index(e1_In, i1_In, e2_In, i2_In, sizeof(STREAM_BankInputEvent_out), 0);
uint64_t t0 = event_for_t0->cases.numIn.timestamp;

STREAM_BankInputEvent_out * event_for_action = (STREAM_BankInputEvent_out *) get_event_at_index(e1_In, i1_In, e2_In, i2_In, sizeof(STREAM_BankInputEvent_out), 0);
int action = event_for_action->cases.numIn.num;

STREAM_BankInputEvent_out * event_for_t1 = (STREAM_BankInputEvent_out *) get_event_at_index(e1_In, i1_In, e2_In, i2_In, sizeof(STREAM_BankInputEvent_out), 1);
uint64_t t1 = event_for_t1->cases.numIn.timestamp;

STREAM_BankInputEvent_out * event_for_acc = (STREAM_BankInputEvent_out *) get_event_at_index(e1_In, i1_In, e2_In, i2_In, sizeof(STREAM_BankInputEvent_out), 1);
int acc = event_for_acc->cases.numIn.num;

STREAM_BankInputEvent_out * event_for_t3 = (STREAM_BankInputEvent_out *) get_event_at_index(e1_In, i1_In, e2_In, i2_In, sizeof(STREAM_BankInputEvent_out), 2);
uint64_t t3 = event_for_t3->cases.numIn.timestamp;

STREAM_BankInputEvent_out * event_for_amnt = (STREAM_BankInputEvent_out *) get_event_at_index(e1_In, i1_In, e2_In, i2_In, sizeof(STREAM_BankInputEvent_out), 2);
int amnt = event_for_amnt->cases.numIn.num;
    ...
    arbiter_outevent->head.id = (*arbiter_counter)++;
    ((STREAM_BankEvent_out *) arbiter_outevent)->cases.montransfer.from_ = from_;
((STREAM_BankEvent_out *) arbiter_outevent)->cases.montransfer.to_ = to_;
((STREAM_BankEvent_out *) arbiter_outevent)->cases.montransfer.amount = amnt;
((STREAM_BankEvent_out *) arbiter_outevent)->cases.montransfer.success = true;
...
current_rule_set = SWITCH_TO_RULE_SET_aligning;
	...
	shm_arbiter_buffer_drop(BUFFER_In, 3);
            ...
            int TEMPARR25[] = {BANKOUTPUTEVENT_TRANSFER,BANKOUTPUTEVENT_TRANSFERSUCCESS};

            
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR25, 2)) {
                    if (check_at_least_n_events(count_In, 3)) {
                        
            STREAM_BankOutputEvent_out * event_for_t2 = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 0);
uint64_t t2 = event_for_t2->cases.transfer.timestamp;

STREAM_BankOutputEvent_out * event_for_f = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 0);
...
current_rule_set = SWITCH_TO_RULE_SET_aligning;
            ...
            int TEMPARR26[] = {BANKOUTPUTEVENT_TRANSFER,BANKOUTPUTEVENT_SELECTEDACCOUNT};
int TEMPARR27[] = {BANKINPUTEVENT_NUMIN,BANKINPUTEVENT_NUMIN,BANKINPUTEVENT_NUMIN};

            
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR26, 2)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR27, 3)) {
...
((STREAM_BankEvent_out *) arbiter_outevent)->cases.montransfer.to_ = to_;
((STREAM_BankEvent_out *) arbiter_outevent)->cases.montransfer.amount = amnt;
((STREAM_BankEvent_out *) arbiter_outevent)->cases.montransfer.success = false;
...
current_rule_set = SWITCH_TO_RULE_SET_aligning;
	...
	shm_arbiter_buffer_drop(BUFFER_In, 3);
            ...
            int TEMPARR28[] = {BANKOUTPUTEVENT_TRANSFER,BANKOUTPUTEVENT_SELECTEDACCOUNT};

            
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR28, 2)) {
                    if (check_at_least_n_events(count_In, 3)) {
...
STREAM_BankOutputEvent_out * event_for_acc = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 1);
...
current_rule_set = SWITCH_TO_RULE_SET_aligning;
            ...
            int TEMPARR29[] = {BANKOUTPUTEVENT_TRANSFER,BANKOUTPUTEVENT_HOLE};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR29, 2)) {
...
int TEMPARR31[] = {BANKINPUTEVENT_NUMIN,BANKINPUTEVENT_HOLE};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR30, 1)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR31, 2)) {
...
int TEMPARR33[] = {BANKINPUTEVENT_NUMIN,BANKINPUTEVENT_OTHERIN};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR32, 1)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR33, 2)) {
...
int TEMPARR35[] = {BANKINPUTEVENT_NUMIN};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR34, 2)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR35, 1)) {
                    
            STREAM_BankOutputEvent_out * event_for_t2 = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 0);
uint64_t t2 = event_for_t2->cases.balance.timestamp;

STREAM_BankOutputEvent_out * event_for_acc = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 0);
int acc = event_for_acc->cases.balance.account;

STREAM_BankOutputEvent_out * event_for_t3 = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 1);
uint64_t t3 = event_for_t3->cases.numOut.timestamp;

STREAM_BankOutputEvent_out * event_for_amnt = (STREAM_BankOutputEvent_out *) get_event_at_index(e1_Out, i1_Out, e2_Out, i2_Out, sizeof(STREAM_BankOutputEvent_out), 1);
int amnt = event_for_amnt->cases.numOut.num;

STREAM_BankInputEvent_out * event_for_t1 = (STREAM_BankInputEvent_out *) get_event_at_index(e1_In, i1_In, e2_In, i2_In, sizeof(STREAM_BankInputEvent_out), 0);
uint64_t t1 = event_for_t1->cases.numIn.timestamp;
...
int action = event_for_action->cases.numIn.num;
    ...
    arbiter_outevent->head.id = (*arbiter_counter)++;
    ((STREAM_BankEvent_out *) arbiter_outevent)->cases.monbalance.account = acc;
((STREAM_BankEvent_out *) arbiter_outevent)->cases.monbalance.balance = amnt;
...
current_rule_set = SWITCH_TO_RULE_SET_aligning;
	...
	shm_arbiter_buffer_drop(BUFFER_In, 1);
            ...
            int TEMPARR36[] = {BANKOUTPUTEVENT_BALANCE,BANKOUTPUTEVENT_HOLE};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR36, 2)) {
...
current_rule_set = SWITCH_TO_RULE_SET_aligning;
            ...
            int TEMPARR37[] = {BANKOUTPUTEVENT_DEPOSITTO,BANKOUTPUTEVENT_DEPOSITSUCCESS};
int TEMPARR38[] = {BANKINPUTEVENT_NUMIN,BANKINPUTEVENT_NUMIN};

            
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR37, 2)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR38, 2)) {
...
STREAM_BankInputEvent_out * event_for_t3 = (STREAM_BankInputEvent_out *) get_event_at_index(e1_In, i1_In, e2_In, i2_In, sizeof(STREAM_BankInputEvent_out), 1);
uint64_t t3 = event_for_t3->cases.numIn.timestamp;

STREAM_BankInputEvent_out * event_for_amnt = (STREAM_BankInputEvent_out *) get_event_at_index(e1_In, i1_In, e2_In, i2_In, sizeof(STREAM_BankInputEvent_out), 1);
int amnt = event_for_amnt->cases.numIn.num;
    ...
    arbiter_outevent->head.id = (*arbiter_counter)++;
    ((STREAM_BankEvent_out *) arbiter_outevent)->cases.deposit.account = acc;
((STREAM_BankEvent_out *) arbiter_outevent)->cases.deposit.amount = amnt;
...
int TEMPARR40[] = {BANKINPUTEVENT_NUMIN,BANKINPUTEVENT_NUMIN};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR39, 2)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR40, 2)) {
...
int TEMPARR42[] = {BANKINPUTEVENT_NUMIN,BANKINPUTEVENT_HOLE};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR41, 2)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR42, 2)) {
...
current_rule_set = SWITCH_TO_RULE_SET_aligning;
	...
	shm_arbiter_buffer_drop(BUFFER_In, 1);
            ...
            int TEMPARR43[] = {BANKOUTPUTEVENT_DEPOSITTO,BANKOUTPUTEVENT_DEPOSITSUCCESS};
int TEMPARR44[] = {BANKINPUTEVENT_NUMIN,BANKINPUTEVENT_HOLE};

            
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR43, 2)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR44, 2)) {
...
current_rule_set = SWITCH_TO_RULE_SET_aligning;
	...
	shm_arbiter_buffer_drop(BUFFER_In, 1);
            ...
            int TEMPARR45[] = {BANKOUTPUTEVENT_DEPOSITTO,BANKOUTPUTEVENT_HOLE};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR45, 2)) {
...
current_rule_set = SWITCH_TO_RULE_SET_aligning;
            ...
            int TEMPARR46[] = {BANKOUTPUTEVENT_WITHDRAW,BANKOUTPUTEVENT_WITHDRAWSUCCESS};
int TEMPARR47[] = {BANKINPUTEVENT_NUMIN,BANKINPUTEVENT_NUMIN};

            
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR46, 2)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR47, 2)) {
...
STREAM_BankInputEvent_out * event_for_act = (STREAM_BankInputEvent_out *) get_event_at_index(e1_In, i1_In, e2_In, i2_In, sizeof(STREAM_BankInputEvent_out), 0);
...
int amnt = event_for_amnt->cases.numIn.num;
    ...
    arbiter_outevent->head.id = (*arbiter_counter)++;
    ((STREAM_BankEvent_out *) arbiter_outevent)->cases.monwithdraw.account = acc;
((STREAM_BankEvent_out *) arbiter_outevent)->cases.monwithdraw.amount = amnt;
((STREAM_BankEvent_out *) arbiter_outevent)->cases.monwithdraw.success = true;
...
int TEMPARR49[] = {BANKINPUTEVENT_NUMIN,BANKINPUTEVENT_NUMIN};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR48, 2)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR49, 2)) {
...
int amnt = event_for_amnt->cases.numIn.num;
    ...
    ((STREAM_BankEvent_out *) arbiter_outevent)->cases.monwithdraw.account = acc;
((STREAM_BankEvent_out *) arbiter_outevent)->cases.monwithdraw.amount = amnt;
((STREAM_BankEvent_out *) arbiter_outevent)->cases.monwithdraw.success = false;
...
current_rule_set = SWITCH_TO_RULE_SET_aligning;
	...
	shm_arbiter_buffer_drop(BUFFER_In, 2);
            ...
            int TEMPARR50[] = {BANKOUTPUTEVENT_WITHDRAW,BANKOUTPUTEVENT_WITHDRAWSUCCESS};
int TEMPARR51[] = {BANKINPUTEVENT_NUMIN,BANKINPUTEVENT_HOLE};

            
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR50, 2)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR51, 2)) {
...
int TEMPARR53[] = {BANKINPUTEVENT_NUMIN,BANKINPUTEVENT_HOLE};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR52, 2)) {
                    
                if (are_events_in_head(e1_In, i1_In, e2_In, i2_In, 
                count_In, sizeof(STREAM_BankInputEvent_out), TEMPARR53, 2)) {
...
current_rule_set = SWITCH_TO_RULE_SET_aligning;
	...
	shm_arbiter_buffer_drop(BUFFER_In, 1);
            ...
            int TEMPARR54[] = {BANKOUTPUTEVENT_WITHDRAW,BANKOUTPUTEVENT_HOLE};
                ...
                if (are_events_in_head(e1_Out, i1_Out, e2_Out, i2_Out, 
                count_Out, sizeof(STREAM_BankOutputEvent_out), TEMPARR54, 2)) {
...
current_rule_set = SWITCH_TO_RULE_SET_aligning;
            ...
            _Bool ok = 1;
if (count_Out >= 2) {if (count_In >= 3) {	ok = 0;
}}if (ok == 0) {
	fprintf(stderr, "Prefix of 'Out':\n");
	count_Out = shm_arbiter_buffer_peek(BUFFER_Out, 5, &e1_Out, &i1_Out, &e2_Out, &i2_Out);
	print_buffer_prefix(BUFFER_Out, 1, i1_Out + i2_Out, count_Out, e1_Out, i1_Out, e2_Out, i2_Out);
	...
	print_buffer_prefix(BUFFER_In, 0, i1_In + i2_In, count_In, e1_In, i1_In, e2_In, i2_In);
fprintf(stderr, "No rule in rule set '{rule_set_name}' matched even though there was enough events, CYCLING WITH NO PROGRESS!\n");abort();}
if (++RULE_SET_working_nomatch_cnt > 8000000) {        	RULE_SET_working_nomatch_cnt = 0;	fprintf(stderr, "\033[31mRule set 'working' cycles long time without progress\033[0m\n");	fprintf(stderr, "Prefix of 'Out':\n");
	count_Out = shm_arbiter_buffer_peek(BUFFER_Out, 5, &e1_Out, &i1_Out, &e2_Out, &i2_Out);
	print_buffer_prefix(BUFFER_Out, 1, i1_Out + i2_Out, count_Out, e1_Out, i1_Out, e2_Out, i2_Out);
	fprintf(stderr, "Prefix of 'In':\n");
	count_In = shm_arbiter_buffer_peek(BUFFER_In, 5, &e1_In, &i1_In, &e2_In, &i2_In);
...
int main(int argc, char **argv) {
 ...
 tessla_in = fopen("/tmp/tessla.in", "w");
	...
	BUFFER_In = shm_arbiter_buffer_create(EV_SOURCE_In,  sizeof(STREAM_BankInputEvent_out), 10000);

	// register events in In
	if (shm_stream_register_event(EV_SOURCE_In, "numIn", BANKINPUTEVENT_NUMIN) < 0) {
		fprintf(stderr, "Failed registering event numIn for stream In : BankInputEvent\n");
		fprintf(stderr, "Available events:\n");
		shm_stream_dump_events(EV_SOURCE_In);
		abort();
	}
	if (shm_stream_register_event(EV_SOURCE_In, "otherIn", BANKINPUTEVENT_OTHERIN) < 0) {
		fprintf(stderr, "Failed registering event otherIn for stream In : BankInputEvent\n");
		fprintf(stderr, "Available events:\n");
		shm_stream_dump_events(EV_SOURCE_In);
		abort();
	}
	...
	BUFFER_Out = shm_arbiter_buffer_create(EV_SOURCE_Out,  sizeof(STREAM_BankOutputEvent_out), 10000);

	// register events in Out
	if (shm_stream_register_event(EV_SOURCE_Out, "balance", BANKOUTPUTEVENT_BALANCE) < 0) {
		fprintf(stderr, "Failed registering event balance for stream Out : BankOutputEvent\n");
		fprintf(stderr, "Available events:\n");
		shm_stream_dump_events(EV_SOURCE_Out);
		abort();
	}
	if (shm_stream_register_event(EV_SOURCE_Out, "depositTo", BANKOUTPUTEVENT_DEPOSITTO) < 0) {
		fprintf(stderr, "Failed registering event depositTo for stream Out : BankOutputEvent\n");
		fprintf(stderr, "Available events:\n");
		shm_stream_dump_events(EV_SOURCE_Out);
		abort();
	}
	if (shm_stream_register_event(EV_SOURCE_Out, "withdraw", BANKOUTPUTEVENT_WITHDRAW) < 0) {
		fprintf(stderr, "Failed registering event withdraw for stream Out : BankOutputEvent\n");
		fprintf(stderr, "Available events:\n");
		shm_stream_dump_events(EV_SOURCE_Out);
		abort();
	}
	if (shm_stream_register_event(EV_SOURCE_Out, "transfer", BANKOUTPUTEVENT_TRANSFER) < 0) {
		fprintf(stderr, "Failed registering event transfer for stream Out : BankOutputEvent\n");
		fprintf(stderr, "Available events:\n");
		shm_stream_dump_events(EV_SOURCE_Out);
		abort();
	}
	if (shm_stream_register_event(EV_SOURCE_Out, "depositSuccess", BANKOUTPUTEVENT_DEPOSITSUCCESS) < 0) {
		fprintf(stderr, "Failed registering event depositSuccess for stream Out : BankOutputEvent\n");
		fprintf(stderr, "Available events:\n");
		shm_stream_dump_events(EV_SOURCE_Out);
		abort();
	}
	if (shm_stream_register_event(EV_SOURCE_Out, "depositFail", BANKOUTPUTEVENT_DEPOSITFAIL) < 0) {
		fprintf(stderr, "Failed registering event depositFail for stream Out : BankOutputEvent\n");
		fprintf(stderr, "Available events:\n");
		shm_stream_dump_events(EV_SOURCE_Out);
		abort();
	}
	if (shm_stream_register_event(EV_SOURCE_Out, "withdrawSuccess", BANKOUTPUTEVENT_WITHDRAWSUCCESS) < 0) {
		fprintf(stderr, "Failed registering event withdrawSuccess for stream Out : BankOutputEvent\n");
		fprintf(stderr, "Available events:\n");
		shm_stream_dump_events(EV_SOURCE_Out);
		abort();
	}
	if (shm_stream_register_event(EV_SOURCE_Out, "withdrawFail", BANKOUTPUTEVENT_WITHDRAWFAIL) < 0) {
		fprintf(stderr, "Failed registering event withdrawFail for stream Out : BankOutputEvent\n");
		fprintf(stderr, "Available events:\n");
		shm_stream_dump_events(EV_SOURCE_Out);
		abort();
	}
	if (shm_stream_register_event(EV_SOURCE_Out, "transferSuccess", BANKOUTPUTEVENT_TRANSFERSUCCESS) < 0) {
		fprintf(stderr, "Failed registering event transferSuccess for stream Out : BankOutputEvent\n");
		fprintf(stderr, "Available events:\n");
		shm_stream_dump_events(EV_SOURCE_Out);
		abort();
	}
	if (shm_stream_register_event(EV_SOURCE_Out, "selectedAccount", BANKOUTPUTEVENT_SELECTEDACCOUNT) < 0) {
		fprintf(stderr, "Failed registering event selectedAccount for stream Out : BankOutputEvent\n");
		fprintf(stderr, "Available events:\n");
		shm_stream_dump_events(EV_SOURCE_Out);
		abort();
	}
	if (shm_stream_register_event(EV_SOURCE_Out, "logout", BANKOUTPUTEVENT_LOGOUT) < 0) {
		fprintf(stderr, "Failed registering event logout for stream Out : BankOutputEvent\n");
		fprintf(stderr, "Available events:\n");
		shm_stream_dump_events(EV_SOURCE_Out);
		abort();
	}
	if (shm_stream_register_event(EV_SOURCE_Out, "numOut", BANKOUTPUTEVENT_NUMOUT) < 0) {
		fprintf(stderr, "Failed registering event numOut for stream Out : BankOutputEvent\n");
		fprintf(stderr, "Available events:\n");
		shm_stream_dump_events(EV_SOURCE_Out);
		abort();
	}
	...
	shm_arbiter_buffer_set_active(BUFFER_Out, true);
 	...
 	monitor_buffer = shm_monitor_buffer_create(sizeof(STREAM_BankEvent_out), 10000);
	...
	thrd_create(&THREAD_Out, PERF_LAYER_Out,BUFFER_Out);
     ...
     thrd_create(&ARBITER_THREAD, arbiter, 0);
		...
		if (received_event->head.kind == 2) {
			int account = received_event->cases.monbalance.account;
			int balance = received_event->cases.monbalance.balance;

		  if (true ) {
		      fprintf(tessla_in, "%lu: monbalance = (%d, %d)\n", ++timestamp, account, balance);
     
		  }
		}
        
		if (received_event->head.kind == 3) {
			int account = received_event->cases.deposit.account;
			int amount = received_event->cases.deposit.amount;

		  if (true ) {
		      fprintf(tessla_in, "%lu: deposit = (%d, %d)\n", ++timestamp, account, amount);
     
		  }
		}
        
		if (received_event->head.kind == 4) {
			int account = received_event->cases.monwithdraw.account;
			int amount = received_event->cases.monwithdraw.amount;
			bool success = received_event->cases.monwithdraw.success;

		  if (true ) {
		      fprintf(tessla_in, "%lu: monwithdraw = (%d, %d, %d)\n", ++timestamp, account, amount, success);
     
		  }
		}
        
		if (received_event->head.kind == 5) {
			int from_ = received_event->cases.montransfer.from_;
			int to_ = received_event->cases.montransfer.to_;
			int amount = received_event->cases.montransfer.amount;
			bool success = received_event->cases.montransfer.success;

		  if (true ) {
		      fprintf(tessla_in, "%lu: montransfer = (%d, %d, %d, %d)\n", ++timestamp, from_, to_, amount, success);
		...
		if (received_event->head.kind == 1) {
		  ...
		  if (true ) {
		      fprintf(tessla_in, "THERE IS A HOLE\n");
