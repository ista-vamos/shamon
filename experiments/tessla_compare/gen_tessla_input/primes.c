...
int process(int n, int p, int pos, int opos) {
    if (pos < opos) {
        ...
        if (intmap_get(&buf, n, &oval)) {
            ...
            arbiter_outevent =
                (STREAM_NumberPairs_out *)shm_monitor_buffer_write_ptr(
                    monitor_buffer);
            arbiter_outevent->head.kind = 2;
            arbiter_outevent->head.id   = (*arbiter_counter)++;
            ((STREAM_NumberPairs_out *)arbiter_outevent)->cases.NumberPair.i =
                n;
            ((STREAM_NumberPairs_out *)arbiter_outevent)->cases.NumberPair.n =
                oval;
            ((STREAM_NumberPairs_out *)arbiter_outevent)->cases.NumberPair.m =
                p;
...
bool Ps_ORDER_EXP(void *args1, void *args2) {
    return ((STREAM_Primes_ARGS *)args1)->pos >
           ((STREAM_Primes_ARGS *)args2)->pos;
}
...
static void print_buffer_prefix(shm_arbiter_buffer *b, int src_idx,
    ...
    while (i < i1) {
        shm_event *ev = (shm_event *)(e1);
        fprintf(stderr, "  %d: {id: %5lu, kind: %3lu", ++n, shm_event_id(ev),
                shm_event_kind(ev));
        if (src_idx != -1)
            fprintf(stderr, " -> %-12s",
                    get_event_name(src_idx, shm_event_kind(ev)));
...
void print_buffers_state() {
    ...
    count = shm_arbiter_buffer_peek(BUFFER_P0, 10, (void **)&e1, &i1,
                                    (void **)&e2, &i2);
    print_buffer_prefix(BUFFER_P0, 0, i1 + i2, count, e1, i1, e2, i2);
    fprintf(stderr, "Prefix of 'P1':\n");
    count = shm_arbiter_buffer_peek(BUFFER_P1, 10, (void **)&e1, &i1,
                                    (void **)&e2, &i2);
...
int RULE_SET_rs() {
    ...
    char  *e2_P0;
    size_t i2_P0;
    int    count_P0 = shm_arbiter_buffer_peek(BUFFER_P0, 1, (void **)&e1_P0,
                                              &i1_P0, (void **)&e2_P0, &i2_P0);
    char  *e1_P1;
    size_t i1_P1;
    char  *e2_P1;
    size_t i2_P1;
    int    count_P1 = shm_arbiter_buffer_peek(BUFFER_P1, 1, (void **)&e1_P1,
                                              &i1_P1, (void **)&e2_P1, &i2_P1);

    int TEMPARR0[] = {PRIMES_PRIME};
    int TEMPARR1[] = {PRIMES_PRIME};

    if (are_events_in_head(e1_P0, i1_P0, e2_P0, i2_P0, count_P0,
                           sizeof(STREAM_Primes_out), TEMPARR0, 1)) {

        if (are_events_in_head(e1_P1, i1_P1, e2_P1, i2_P1, count_P1,
                               sizeof(STREAM_Primes_out), TEMPARR1, 1)) {

            STREAM_Primes_out *event_for_ln =
                (STREAM_Primes_out *)get_event_at_index(
                    e1_P0, i1_P0, e2_P0, i2_P0, sizeof(STREAM_Primes_out), 0);
            int ln = event_for_ln->cases.prime.n;

            STREAM_Primes_out *event_for_lp =
                (STREAM_Primes_out *)get_event_at_index(
                    e1_P0, i1_P0, e2_P0, i2_P0, sizeof(STREAM_Primes_out), 0);
            int lp = event_for_lp->cases.prime.p;

            STREAM_Primes_out *event_for_rn =
                (STREAM_Primes_out *)get_event_at_index(
                    e1_P1, i1_P1, e2_P1, i2_P1, sizeof(STREAM_Primes_out), 0);
            int rn = event_for_rn->cases.prime.n;

            STREAM_Primes_out *event_for_rp =
                (STREAM_Primes_out *)get_event_at_index(
                    e1_P1, i1_P1, e2_P1, i2_P1, sizeof(STREAM_Primes_out), 0);
            ...
            if (true) {
                ...
                if (ln == rn) {
                    ...
                    arbiter_outevent->head.id   = (*arbiter_counter)++;
                    ((STREAM_NumberPairs_out *)arbiter_outevent)
                        ->cases.NumberPair.i = ln;
                    ((STREAM_NumberPairs_out *)arbiter_outevent)
                        ->cases.NumberPair.n = lp;
                    ((STREAM_NumberPairs_out *)arbiter_outevent)
                        ->cases.NumberPair.m = rp;
                ...
                } else if (stream_args_P_0->pos < stream_args_P_1->pos) {
                    left_processed += 1;
                    stream_args_P_0->pos = process(ln, lp, stream_args_P_0->pos,
                                                   stream_args_P_1->pos);
                    shm_arbiter_buffer_drop(BUFFER_P0, 1);

                } else {
                    right_processed += 1;
                    stream_args_P_1->pos = process(rn, rp, stream_args_P_1->pos,
                                                   stream_args_P_0->pos);
                    shm_arbiter_buffer_drop(BUFFER_P1, 1);
                }
    ...
    if (is_selection_successful) {

        {
            ...
            STREAM_Primes_ARGS *stream_args_F =
                (STREAM_Primes_ARGS *)chosen_streams[0]->args;
            char  *e1_F;
            size_t i1_F;
            char  *e2_F;
            size_t i2_F;
            int count_F = shm_arbiter_buffer_peek(BUFFER_F, 1, (void **)&e1_F,
                                                  &i1_F, (void **)&e2_F, &i2_F);
            shm_stream         *S        = chosen_streams[1]->stream;
            shm_arbiter_buffer *BUFFER_S = chosen_streams[1]->buffer;
            STREAM_Primes_ARGS *stream_args_S =
                (STREAM_Primes_ARGS *)chosen_streams[1]->args;

            int TEMPARR2[] = {PRIMES_PRIME};

            if (are_events_in_head(e1_F, i1_F, e2_F, i2_F, count_F,
                                   sizeof(STREAM_Primes_out), TEMPARR2, 1)) {

                STREAM_Primes_out *event_for_n =
                    (STREAM_Primes_out *)get_event_at_index(
                        e1_F, i1_F, e2_F, i2_F, sizeof(STREAM_Primes_out), 0);
                int n = event_for_n->cases.prime.n;

                STREAM_Primes_out *event_for_p =
                    (STREAM_Primes_out *)get_event_at_index(
                        e1_F, i1_F, e2_F, i2_F, sizeof(STREAM_Primes_out), 0);
                ...
                if (count < 10 || stream_args_F->pos <= stream_args_S->pos) {
                    ...
                    stream_args_F->pos =
                        process(n, p, stream_args_F->pos, stream_args_S->pos);
                    if (F == EV_SOURCE_P_0) {
                        left_processed++;
                    } else {
                        right_processed++;
            ...
            if (are_events_in_head(e1_F, i1_F, e2_F, i2_F, count_F,
                                   sizeof(STREAM_Primes_out), TEMPARR3, 1)) {
                ...
                if (true) {
                    ...
                    printf("THERE IS A HOLE");
                    if (stream_args_F->pos < stream_args_S->pos) {
                        int removed =
                            intmap_remove_upto(&buf, stream_args_F->pos + n);
                        count -= removed;
                    }
    ...
    if (ok == 0) {
        fprintf(stderr, "Prefix of 'P0':\n");
        count_P0 = shm_arbiter_buffer_peek(BUFFER_P0, 5, &e1_P0, &i1_P0, &e2_P0,
                                           &i2_P0);
        print_buffer_prefix(BUFFER_P0, 0, i1_P0 + i2_P0, count_P0, e1_P0, i1_P0,
                            e2_P0, i2_P0);
        fprintf(stderr, "Prefix of 'P1':\n");
        count_P1 = shm_arbiter_buffer_peek(BUFFER_P1, 5, &e1_P1, &i1_P1, &e2_P1,
                                           &i2_P1);
        print_buffer_prefix(BUFFER_P1, 0, i1_P1 + i2_P1, count_P1, e1_P1, i1_P1,
                            e2_P1, i2_P1);
        fprintf(stderr,
                "No rule in rule set '{rule_set_name}' matched even though "
                "there was enough events, CYCLING WITH NO PROGRESS!\n");
        abort();
    }
    if (++RULE_SET_rs_nomatch_cnt > 8000000) {
        RULE_SET_rs_nomatch_cnt = 0;
        fprintf(
            stderr,
            "\033[31mRule set 'rs' cycles long time without progress\033[0m\n");
        fprintf(stderr, "Prefix of 'P0':\n");
        count_P0 = shm_arbiter_buffer_peek(BUFFER_P0, 5, &e1_P0, &i1_P0, &e2_P0,
                                           &i2_P0);
        print_buffer_prefix(BUFFER_P0, 0, i1_P0 + i2_P0, count_P0, e1_P0, i1_P0,
...
int arbiter() {
    ...
    while (!are_streams_done()) {
        ...
        if (no_matches_count == no_consecutive_matches_limit) {
            ...
            printf("LEFT: processed %d events (%d compared, %d dropped (in %d "
                   "holes), %d skipped) \n",
                   left_processed, compared, left_drops, left_holes,
                   left_skipped);

            printf("RIGHT: processed %d events (%d compared, %d dropped (in %d "
                   "holes), %d skipped) \n",
                   right_processed, compared, right_drops, right_holes,
                   right_skipped);
...
static void sig_handler(int sig) {
    ...
    shm_stream_detach(EV_SOURCE_P_0);
...
int main(int argc, char **argv) {
    ...
    *arbiter_counter = 10;
    ...
    BUFFER_P0     = shm_arbiter_buffer_create(EV_SOURCE_P_0,
                                              sizeof(STREAM_Primes_out), 40000);

    // register events in P_0
    if (shm_stream_register_event(EV_SOURCE_P_0, "prime", PRIMES_PRIME) < 0) {
        fprintf(stderr,
                "Failed registering event prime for stream P_0 : Primes\n");
    ...
    BUFFER_P1     = shm_arbiter_buffer_create(EV_SOURCE_P_1,
                                              sizeof(STREAM_Primes_out), 40000);

    // register events in P_1
    if (shm_stream_register_event(EV_SOURCE_P_1, "prime", PRIMES_PRIME) < 0) {
        fprintf(stderr,
                "Failed registering event prime for stream P_1 : Primes\n");
        fprintf(stderr, "Available events:\n");
    ...
    monitor_buffer =
        shm_monitor_buffer_create(sizeof(STREAM_NumberPairs_out), 40000);
    ...
    int left_skipped  = left_processed - compared - left_drops;
    int right_skipped = left_processed - compared - right_drops;
    printf("LEFT: processed %d events (%d compared, %d dropped (in %d holes), "
           "%d skipped) \n",
           left_processed, compared, left_drops, left_holes, left_skipped);

    printf("RIGHT: processed %d events (%d compared, %d dropped (in %d holes), "
           "%d skipped) \n",
