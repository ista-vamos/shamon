...
// dr_emit_flags_t process_basic_block(void *drcontext, void *tag, instrlist_t
...
//         {
//             instr_t *jump_end  =
//             XINST_CREATE_jump(drcontext,opnd_create_instr(next));
//             instrlist_preinsert(bb, instr, jump_end);

//             instr_t *comp0 =
//             XINST_CREATE_cmp(drcontext,opnd_create_reg(DR_REG_RAX),
//             opnd_create_immed_int64(0)); instr_t *comp1 =
//             XINST_CREATE_cmp(drcontext,opnd_create_reg(DR_REG_RAX),
//             opnd_create_immed_int64(1)); instr_t *jump_not1 =
//             XINST_CREATE_jump_cond(drcontext, DR_PRED_NE,
//             opnd_create_instr(comp0)); instr_t *jump_not0 =
//             XINST_CREATE_jump_cond(drcontext, DR_PRED_NE,
//             opnd_create_instr(instr)); instr_t *jump_end1 =
//             XINST_CREATE_jump(drcontext,opnd_create_instr(next));
...
static bool event_pre_syscall(void *drcontext, int sysnum) {
    ...
    // per_thread_t *data = (per_thread_t *)drmgr_get_cls_field(drcontext,
    // tcls_idx); data->fd = fd; /* store the fd for post-event */

    // dr_printf("\033[0;36m[write@%d (%u)]: %.*s\033[0m", fd, len, len,
    // output);
