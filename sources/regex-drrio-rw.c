...
static void usage_and_exit(int ret) {
	printf("Usage: monitor PID shmkey outsplit name expr sig [name expr sig] ... -- insplit name expr sig [name expr sig] ...\n");
	exit(ret);
}
...
int monitoring_thread(void *arg)
{
	...
	while (monitoring_active)
	{
		...
		for(;line!=NULL;line=(free(line),buf_get_upto(&write_msg, outdelim)))
        {
            len=strlen(line);
            ...
            for (int i = 0; i < (int)exprs_num_out; ++i) {
                ...
                for (const char *o = signatures_out[i]; *o && m <= MAXMATCH; ++o, ++m) {
                    if (m > 1)
                        printf(", ");

                    if (*o == 'L') { /* user wants the whole line */
                        printf("'%s'", line);
                        addr = buffer_partial_push_str(shm_out, addr, ev_out.base.id, line);
                        continue;
                    }
                    if (*o != 'M') {
                    ...
                    if (*o == 'M') { /* user wants the whole match */
                        ...
                        tmpline[len] = '\0';
                        addr = buffer_partial_push_str(shm_out, addr, ev_out.base.id, tmpline);
                        printf("'%s'",  tmpline);
                        continue;
                    } else {
                    ...
                    switch(*o) {
                        case 'c':
                            ...
                            addr = buffer_partial_push(shm_out, addr,
                                                    (char*)(line + matches[m].rm_eo),
                                                    sizeof(op.c));
                            break;
                        case 'i': op.i = atoi(tmpline);
                                printf("%d", op.i);
                                addr = buffer_partial_push(shm_out, addr, &op.i, sizeof(op.i));
                                break;
                        case 'l': op.l = atol(tmpline);
                                printf("%ld", op.l);
                                addr = buffer_partial_push(shm_out, addr, &op.l, sizeof(op.l));
                                break;
                        case 'f': op.f = atof(tmpline);
                                printf("%lf", op.f);
                                addr = buffer_partial_push(shm_out, addr, &op.f, sizeof(op.f));
                                break;
                        case 'd': op.d = strtod(tmpline, NULL);
                                printf("%lf", op.d);
                                addr = buffer_partial_push(shm_out, addr, &op.d, sizeof(op.d));
                                break;
		...
		for(;line!=NULL;line=(free(line),buf_get_upto(&read_msg, indelim)))
        {
            len=strlen(line);
            ...
            for (int i = 0; i < (int)exprs_num_in; ++i) {
                ...
                /* push the arguments of the event */
                for (const char *o = signatures_in[i]; *o && m <= MAXMATCH; ++o, ++m) {
                    if (m > 1)
                        printf(", ");

                    if (*o == 'L') { /* user wants the whole line */
                        printf("'%s'", line);
                        addr = buffer_partial_push_str(shm_in, addr, ev_in.base.id, line);
                        continue;
                    }
                    if (*o != 'M') {
                        if ((int)matches[m].rm_so < 0) {
                    ...
                    if (*o == 'M') { /* user wants the whole match */
                        ...
                        strncpy(tmpline, line+matches[0].rm_so, len);
                        tmpline[len] = '\0';
                        addr = buffer_partial_push_str(shm_in, addr, ev_in.base.id, tmpline);
                        printf("'%s'",  tmpline);
                        continue;
                    } else {
                        strncpy(tmpline, line+matches[m].rm_so, len);
                    ...
                    switch(*o) {
                        case 'c':
                            assert(len == 1);
                            printf("%c", *(char*)(line + matches[m].rm_eo));
                            addr = buffer_partial_push(shm_in, addr,
                                                    (char*)(line + matches[m].rm_eo),
                                                    sizeof(op.c));
                            break;
                        case 'i': op.i = atoi(tmpline);
                                printf("%d", op.i);
                                addr = buffer_partial_push(shm_in, addr, &op.i, sizeof(op.i));
                                break;
                        case 'l': op.l = atol(tmpline);
                                printf("%ld", op.l);
                                addr = buffer_partial_push(shm_in, addr, &op.l, sizeof(op.l));
                                break;
                        case 'f': op.f = atof(tmpline);
                                printf("%lf", op.f);
                                addr = buffer_partial_push(shm_in, addr, &op.f, sizeof(op.f));
                                break;
                        case 'd': op.d = strtod(tmpline, NULL);
                                printf("%lf", op.d);
                                addr = buffer_partial_push(shm_in, addr, &op.d, sizeof(op.d));
                                break;
                        case 'S': printf("'%s'", tmpline);
                                addr = buffer_partial_push_str(shm_in, addr, ev_in.base.id, tmpline);
                                break;
